    ...
    },
    {
      ...
      config: {
        ...
        validate: {
          query: {
            ...
            type: isA.string().max(32).alphanum().allow(['upgradeSession', 'loginCode']).optional()
          },
          payload: {
      ...
      handler (request, reply) {
        ...
        function setVerifyFunction () {
          ...
          } else if (type && type === 'loginCode') {
            verifyFunction = mailer.sendVerifyLoginCodeEmail
            event = 'confirmation_login_code'
          } else if (email) {
            ...
            return db.deviceFromTokenVerificationId(uid, code)
              ...
              .then(() => {
                if (! isAccountVerification) {
                  ...
                  // Emit the proper metrics depending on whether this was a 2FA style login code
                  // or the sign-in confirmation email link.
                  if (type === 'loginCode') {
                    log.info({
                      op: 'account.login.code.success',
                      uid: uid,
                      code: request.payload.code
                    })
                  } else {
