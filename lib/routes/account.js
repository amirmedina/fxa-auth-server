    ...
    },
    {
      ...
      config: {
        validate: {
          query: {
            keys: isA.boolean().optional(),
            service: validators.service,
            verificationMethod: isA.string().valid(['email', 'email-2fa']).optional()
          },
          payload: {
            email: validators.email().required(),
            authPW: isA.string().min(64).max(64).regex(HEX_STRING).required(),
      ...
      handler: function (request, reply) {
        ...
        function checkEmailAndPassword () {
          ...
          // can actually use it.  If they don't want keys, they don't *have* to verify their
          // their session unless a verification method was explicitly specified. If keys are not
          // requested, the session is created with a non-null tokenVerificationId, so it will
          // still be considered unverified.  This prevents the session from being used for sync
          // unless the user explicitly requests us to send the confirmation email, and completes it.
        ...
        function sendVerificationIfNeeded() {
          ...
          if (doSigninConfirmation) {
            if (verificationMethod === 'email') {
              ...
              // Send an email containing a code to verify login
        ...
        function sendVerifyLoginCodeEmail() {
          ...
          return P.all([request.app.geo, db.accountEmails(sessionToken.uid)])
            ...
            .then(() => request.emitMetricsEvent('email.logincode.sent'))
        }
