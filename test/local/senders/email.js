  ...
  () => {
    ...
    messageTypes.forEach(
      function (type) {
        ...
        if (includes(typesContainLoginCode, type)) {
          it(
            'login code is in email template output for ' + type,
            function () {
              mailer.mailer.sendMail = function (emailConfig) {
                assert.ok(includes(emailConfig.html, message.loginCode))
                assert.ok(includes(emailConfig.text, message.loginCode))
              }
              mailer[type](message)
            }
          )
